...
const MASTER_PREFIX = 'masters-private/';
...
/**
 * Unregistered server-only adapter. Each durable storage fence maps to one immutable object key.
 * It is safe for COS buckets whose versioning is enabled, suspended, or never configured.
 */
export class TencentCosTrustedPhotoshopIngestionPrivateStorage implements TrustedPhotoshopIngestionPrivateStorage {
  ...
  private async store(input: { tenantId: string; operationId: string; slot: string; grant: { objectIdentity: string; fenceToken: string; intentDigest: string }; content: Buffer; mimeType: string; byteLength: number; sha256: string; artifactSlot?: string }): Promise<{ privateLocator: string }> {
    try {
      ...
      const request: TencentCosTrustedPhotoshopIngestionPutRequest = { Bucket: BUCKET, Region: REGION, Key: key, Body: input.content, ContentLength: input.byteLength, ContentType: input.mimeType, 'x-cos-meta-contract': metadata.contract, 'x-cos-meta-identity': metadata.identity, 'x-cos-meta-sha256': metadata.sha256, 'x-cos-meta-length': metadata.length, 'x-cos-meta-mime': metadata.mime, 'x-cos-meta-fence-token-digest': metadata.fenceTokenDigest };
      try { await this.client.putObject(request); } catch (_error: unknown) { /* one verification HEAD is required after an ambiguous PUT outcome */ }
