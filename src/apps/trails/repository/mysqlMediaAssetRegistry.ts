...
import { Actor, DurableCommerceMutation, DurableMediaAsset, DurableMediaAssetArtifactCodec, DurableMediaAssetArtifactDescriptor, DurableMediaAssetRegistryStore, DurableMediaAssetVariantName, ServerApprovedPublicDerivativePublication, WorkspaceMediaAssetPickerItem } from '../types';
...
const variants: DurableMediaAssetVariantName[] = ['grid-960', 'cover-2048', 'preview-4096'];
...
const immutablePublicReference = (assetId: string, name: DurableMediaAssetVariantName, artifact: ArtifactRow): string =>
...
export class MySqlMediaAssetRegistryRepository implements DurableMediaAssetRegistryStore {
  ...
  async approvePublicDerivatives(actor: Actor, input: DurableCommerceMutation & { assetId: string; publication: ServerApprovedPublicDerivativePublication }): Promise<DurableMediaAsset> { if (!['workload-identity', 'static-scoped-key'].includes(input.publication.identityMode) || !opaqueReference.test(input.publication.approvalId)) throw new TrailsSyncInputError('公开衍生物发布审批无效'); return this.mutate(actor, input, 'asset:derivatives:approve', { assetId: input.assetId, approvalId: input.publication.approvalId, identityMode: input.publication.identityMode, expectedResourceVersion: input.expectedResourceVersion }, async transaction => { const row = await this.owned(actor, input.assetId, transaction); if (row.status === 'published') throw new TrailsSyncInputError('已发布资源不能变更公开衍生物'); if (input.expectedResourceVersion !== row.resourceVersion) throw new TrailsMediaAssetRegistryStaleVersionError('资源版本已过期'); const stored = await this.models.artifacts.list({ tenantId: row.tenantId, assetId: row.id }, { transaction }); if (stored.length !== artifactMatrix.length || artifactMatrix.some(expected => !stored.some(item => item.logicalRendition === expected.logicalRendition && item.codec === expected.codec && item.mimeType === expected.mimeType))) throw new TrailsSyncInputError('公开发布前必须具备完整私有衍生物矩阵'); const time = this.now(); for (const name of variants) { const selected = stored.find(item => item.logicalRendition === name && item.codec === 'jpeg'); if (!selected || !positiveSafeInteger(selected.width) || !positiveSafeInteger(selected.height) || !/^[a-f0-9]{64}$/.test(selected.sha256)) throw new TrailsSyncPersistenceError('approved public derivative source invalid'); await this.models.variants.upsert({ tenantId: row.tenantId, assetId: row.id, name, publicReference: immutablePublicReference(row.id, name, selected), width: selected.width, height: selected.height, state: 'ready', createdAt: time, updatedAt: time }, { transaction }); } const next: AssetRow = { ...row, resourceVersion: (BigInt(row.resourceVersion) + BigInt(1)).toString(), updatedAt: time }; const saved = await this.models.assets.cas({ tenantId: row.tenantId, id: row.id, expectedVersion: row.resourceVersion, next }, { transaction }); if (!saved) throw new TrailsMediaAssetRegistryStaleVersionError('资源版本已过期'); return asset(saved); }); }
  ...
  async publish(actor: Actor, input: DurableCommerceMutation & { id: string }): Promise<DurableMediaAsset> { return this.mutate(actor, input, 'asset:publish', { id: input.id, expectedResourceVersion: input.expectedResourceVersion }, async transaction => { const row = await this.owned(actor, input.id, transaction); if (input.expectedResourceVersion !== row.resourceVersion) throw new TrailsMediaAssetRegistryStaleVersionError('资源版本已过期'); const ready = await this.models.variants.list({ tenantId: row.tenantId, assetId: row.id }, { transaction }); const stored = await this.models.artifacts.list({ tenantId: row.tenantId, assetId: row.id }, { transaction }); if (stored.length !== artifactMatrix.length) throw new TrailsSyncInputError('发布前必须具备完整私有衍生物矩阵'); if (ready.length !== variants.length || variants.some(name => !ready.some(item => item.name === name && item.state === 'ready'))) throw new TrailsSyncInputError('发布前必须具备全部审批ready公开variant'); const next: AssetRow = { ...row, status: 'published', resourceVersion: (BigInt(row.resourceVersion) + BigInt(1)).toString(), updatedAt: this.now() }; const saved = await this.models.assets.cas({ tenantId: row.tenantId, id: row.id, expectedVersion: row.resourceVersion, next }, { transaction }); if (!saved) throw new TrailsMediaAssetRegistryStaleVersionError('资源版本已过期'); return asset(saved); }); }
}
