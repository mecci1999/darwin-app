...
`public-derivatives/` is written only by the reviewed production publication worker after it has revalidated an approved private JPEG derivative. Each key is the fixed prefix plus an opaque immutable reference and `.jpg`; actions return only that reference, never an object key or URL. `downloads-private/` and `print-private/` have no assigned Trails mapping: this code does not read, write, list, delete, or otherwise infer a purpose for either prefix. Private COS keys, locators, URLs, bucket details, and credentials remain absent from actions and responses.

## Production media-delivery guard

The verification CLI is intentionally development-only: its package command pins `NODE_ENV=development`. The production publication path is separate: it is disabled unless `TRAILS_MEDIA_PUBLICATION_ENABLED=true`, `TRAILS_MEDIA_SERVER_IDENTITY_MODE=static-scoped-key`, `TRAILS_COS_CREDENTIAL_PROVIDER=static-env`, the fixed bucket/region/namespace identifiers, a CDN distribution identifier, and protected server-only credentials plus an ingestion mapping secret are supplied. `TRAILS_COS_SECURITY_TOKEN` is rejected in this mode. That validation does not expose a COS client, a bucket/object address, or browser credentials. Cloud-console setup must retain a private COS origin, authorize the CDN only for `public-derivatives/`, and give the server identity only the three required prefixes; do not make a bucket public or put credentials in browser configuration.
