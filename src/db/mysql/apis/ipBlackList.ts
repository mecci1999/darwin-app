...
import { IIPBlackListTableAttributes, IPBlackListTable } from '../models/ipBlackList';

/**
 * 批量新增ip地址黑名单
 */
export async function saveOrUpdateIpBlackList(ipList: IIPBlackListTableAttributes[]) {
  const model = await mainConnection.getModel<IPBlackListTable>(DataBaseTableNames.IPBlackList);
  ...
  const existingMap = new Map(existingIPs.map((ec) => [ec?.ipv4 || ec?.ipv6, ec]));
