...
const packageEntries = async () => [
  ...
  { name: 'website/grid-800.v1.jpg', buffer: await image(800, 400) },
  { name: 'website/cover-1600.v1.jpg', buffer: await image(1600, 800) },
  { name: 'website/preview-2048.v1.jpg', buffer: await image(2048, 1024) },
];
...
describe('trusted Photoshop publication ingestion coordinator', () => {
  it('imports once, records the master and nine artifacts, registers a draft asset, and returns only the safe summary', async () => {
    ...
    expect(stored.countForTest()).toBe(10); expect(operations.calls).toEqual(['create', 'claim', 'transition:storage_pending', 'acquire:master', 'recordMaster', 'acquire:grid-800:avif', 'record:grid-800:avif', 'acquire:grid-800:webp', 'record:grid-800:webp', 'acquire:grid-800:jpeg', 'record:grid-800:jpeg', 'acquire:cover-1600:avif', 'record:cover-1600:avif', 'acquire:cover-1600:webp', 'record:cover-1600:webp', 'acquire:cover-1600:jpeg', 'record:cover-1600:jpeg', 'acquire:preview-2048:avif', 'record:preview-2048:avif', 'acquire:preview-2048:webp', 'record:preview-2048:webp', 'acquire:preview-2048:jpeg', 'record:preview-2048:jpeg', 'transition:storage_recorded', 'transition:registry_master_pending', 'transition:registry_master_recorded', 'transition:registry_artifacts_pending', 'transition:completed']);
  ...
  it('replays stable private identities, rejects changed content before mutation, and keeps defensive copies', async () => {
    const storage = new InMemoryTrustedPhotoshopIngestionPrivateStorage(); const content = Buffer.from('verified'); const sha256 = hash(content); const grant = { objectIdentity: 'a'.repeat(64), fenceToken: 'b'.repeat(64) };
 ...
 expect(second).toEqual(first); content[0] ^= 0xff; expect(storage.bufferCopyForTest(grant.objectIdentity)).toEqual(Buffer.from('verified')); await expect(storage.storeMaster({ tenantId: 'tenant', operationId: 'op', grant, content: Buffer.from('changed'), mimeType: 'image/jpeg', byteLength: 7, sha256: hash(Buffer.from('changed')) })).rejects.toThrow('collision'); expect(storage.countForTest()).toBe(1);
  ...
  it('records successful partial storage durably and blocks the ambiguous run', async () => {
    ...
    const services = registry(); expectRedacted(await ingestTrustedPhotoshopPublication(await input(), operations, flaky, services).catch(error => error)); expect(operations.current?.phase).toBe('blocked_ambiguous'); expect(operations.current?.failureClass).toBe('storage'); expect(operations.calls).toContain('record:grid-800:avif'); expect(services.register).not.toHaveBeenCalled(); expect(stored.countForTest()).toBe(2);
  });
