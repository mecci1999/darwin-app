...
import { Actor, DurableMediaAssetRegistryStore, WorkspaceMediaAssetPickerItem } from '../../src/apps/starlight/trails/types';
...
const registry = (overrides: Partial<DurableMediaAssetRegistryStore> = {}): DurableMediaAssetRegistryStore => ({ listWorkspacePicker: jest.fn(async () => [pickerRecord]), listPublic: jest.fn(async () => [pickerRecord]), register: jest.fn(async () => record), registerVariant: jest.fn(async () => record), persistArtifacts: jest.fn(async () => record), publish: jest.fn(async () => ({ ...record, status: 'published' as const })), ...overrides });
...
describe('v2 durable media asset registry actions', () => {
  ...
  it('rejects derivative URLs and object-key-shaped values through the durable action', async () => {
    const durable = registry();
    const response = await trailsActions(star, { repository: new InMemoryTrailsRepository(), durableMediaAssetRegistryStore: durable })['v2.media-assets.variants.register'].handler(ctx(owner, { mutationId: 'm', expectedResourceVersion: '1', assetId: 'asset-1', name: 'cover-1600', publicReference: 'https://bad.example', width: 1600, height: 1000 }) as never);
    expect(response.status).toBe(400);
    expect(durable.registerVariant).not.toHaveBeenCalled();
  });
