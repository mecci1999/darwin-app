...
const migrations = [
  ...
  {
    // Conversion and campaign aggregates intentionally contain only normalized dimensions.
    // No raw UTM values, URLs, referrers, IP addresses or participant data are persisted.
    id: TRAILS_OPERATIONS_ANALYTICS_MIGRATION_ID,
    transactional: false,
    async up({ queryInterface }) {
      const { DataTypes } = require('sequelize');
      if (tableExists(await queryInterface.showAllTables(), 'TrailsAnalyticsDaily')) {
        await queryInterface.changeColumn('TrailsAnalyticsDaily', 'event_name', {
          type: DataTypes.ENUM('page-view', 'content-view', 'outbound-click', 'registration-open', 'registration-submit'),
          allowNull: false,
        });
      }
      await ensureTable(queryInterface, 'TrailsAnalyticsCampaignDaily', {
        tenant_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
        ...
        campaign_source: { type: DataTypes.ENUM('wechat', 'xiaohongshu', 'bilibili', 'instagram', 'other'), allowNull: false, primaryKey: true },
        page_views: { type: DataTypes.BIGINT.UNSIGNED, allowNull: false, defaultValue: 0 },
      });
      await ensureIndex(queryInterface, 'TrailsAnalyticsCampaignDaily', ['tenant_id', 'owner_user_id', 'day'], { name: 'trails_analytics_campaign_day' });
    },
  },
  {
    id: TRAILS_FIELD_PLAN_MIGRATION_ID,
    // This personal planning boundary is append-only in the migration ledger. Payloads are
    // owner-private; audit rows retain only lifecycle/version metadata, never payload JSON.
    transactional: false,
    async up({ queryInterface }) {
      const { DataTypes } = require('sequelize');
      await ensureTable(queryInterface, 'TrailsFieldPlan', {
        tenant_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
        id: { type: DataTypes.STRING(160), allowNull: false, primaryKey: true },
        owner_user_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
        status: { type: DataTypes.ENUM('planned', 'in_field', 'completed', 'cancelled'), allowNull: false },
      ...
      await ensureIndex(queryInterface, 'TrailsFieldPlan', ['tenant_id', 'owner_user_id', 'lifecycle', 'updated_at'], { name: 'trails_field_plan_owner_lifecycle_updated' });
      await ensureTable(queryInterface, 'TrailsFieldPlanOwnerQuota', {
        tenant_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
        owner_user_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
        active_plan_count: { type: DataTypes.SMALLINT.UNSIGNED, allowNull: false, defaultValue: 0 },
        created_at: { type: DataTypes.DATE, allowNull: false, defaultValue: DataTypes.NOW },
        updated_at: { type: DataTypes.DATE, allowNull: false, defaultValue: DataTypes.NOW },
      });
      await ensureTable(queryInterface, 'TrailsFieldPlanMutation', {
        tenant_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
      ...
      await ensureTable(queryInterface, 'TrailsFieldPlanAudit', {
      ...
      await ensureIndex(queryInterface, 'TrailsFieldPlanAudit', ['tenant_id', 'field_plan_id', 'occurred_at'], { name: 'trails_field_plan_audit_resource' });
    },
  },
  {
    // Structured field records are owner-private text only. They intentionally do
    // not use a foreign key so archival of a plan never destroys field history.
    transactional: false,
    id: TRAILS_FIELD_RECORD_MIGRATION_ID,
    async up({ queryInterface }) {
      const { DataTypes } = require('sequelize');
      await ensureTable(queryInterface, 'TrailsFieldRecord', {
        tenant_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
        owner_user_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
        field_plan_id: { type: DataTypes.STRING(160), allowNull: false, primaryKey: true },
        weather_note: { type: DataTypes.STRING(500), allowNull: true },
      ...
      await ensureIndex(queryInterface, 'TrailsFieldRecord', ['tenant_id', 'owner_user_id', 'updated_at'], { name: 'trails_field_record_owner_updated' });
      await ensureTable(queryInterface, 'TrailsFieldRecordMutation', {
        tenant_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
        actor_user_id: { type: DataTypes.STRING(64), allowNull: false, primaryKey: true },
        mutation_id: { type: DataTypes.STRING(160), allowNull: false, primaryKey: true },
        fingerprint: { type: DataTypes.CHAR(64), allowNull: false },
      ...
      await ensureTable(queryInterface, 'TrailsFieldRecordAudit', {
        event_id: { type: DataTypes.STRING(160), allowNull: false, primaryKey: true },
        tenant_id: { type: DataTypes.STRING(64), allowNull: false },
        field_plan_id: { type: DataTypes.STRING(160), allowNull: false },
        actor_user_id: { type: DataTypes.STRING(64), allowNull: false },
